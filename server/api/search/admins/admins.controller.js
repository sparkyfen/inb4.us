...
exports.index = function(req, res) {
  ...
  if(validator.isNull(id)) {
    ...
    admins.searchPartialUsername(username, function (error, reply) {
      if(error) {
        console.log(error);
        return res.status(500).jsonp({message: 'Could not search for admin.'});
      }
      ...
      // TODO If we want more data on the admins, we can update the usernames list with these values.
